...
def get_openai_client(api_key: str, base_url: str = None):
    ...
    """
    if not api_key:
        # Commentato per ridurre output UI
    ...
    try:
        ...
        logging.error(f"Errore durante la creazione del client OpenAI: {e}")
        return None
...
def evaluate_answer(question: str, expected_answer: str, actual_answer: str, 
    ...
    try:
        ...
        logging.error(
            f"Errore API durante la valutazione: {type(e).__name__} - {e}"
        )
        ...
        logging.error(
            f"Errore imprevisto durante la valutazione: {type(e).__name__} - {e}"
        )
        api_details_for_log["error"] = str(e)
        return {
...
def generate_example_answer_with_llm(question: str, client_config: dict, show_api_details: bool = False):
    ...
    try:
        ...
        logging.error(
            f"Errore imprevisto durante la generazione della risposta: {type(e).__name__} - {e}"
        )
        if show_api_details:
            api_details_for_log["error"] = str(e)
        return {
