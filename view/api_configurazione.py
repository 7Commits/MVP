...
from controllers.api_preset_controller import load_presets, save_presets, delete_preset
...
def delete_preset_callback(preset_id):
    ...
    save_presets(st.session_state.api_presets)
        ...
        for index, preset in st.session_state.api_presets.iterrows():
            with st.container():
                ...
                with cols_preset_details[2]:
                    if st.button("🗑️ Elimina", key=f"delete_{preset['id']}", on_click=delete_preset_callback, args=(preset['id'],), type="secondary", use_container_width=True):
                        pass
                st.divider()
