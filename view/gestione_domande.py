...
def save_question_callback(question_id, edited_question, edited_answer, edited_category):
    ...
    if update_question(question_id, domanda=edited_question, risposta_attesa=edited_answer,
...
def import_questions_callback():
    ...
    if 'uploaded_file_content' in st.session_state and st.session_state.uploaded_file_content is not None:
        ...
        if success:
            ...
            # Ricarica le domande dal database per aggiornare lo stato
...
# Carica sempre le domande dal database per la visualizzazione
st.session_state.questions = load_questions()
    ...
    with st.form("add_question_form"):
        ...
        if submitted:
            if domanda and risposta_attesa:
                ...
                question_id = add_question(domanda=domanda, risposta_attesa=risposta_attesa,
                                           categoria=categoria)
